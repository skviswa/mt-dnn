...
def build_data(
        ...
        with open(dump_path, "w", encoding="utf-8") as writer:
            for idx, sample in enumerate(data):
                ...
                input_ids, input_mask, type_ids = feature_extractor(
                    tokenizer, premise, max_length=max_seq_len
                )
                features = {
                    "uid": ids,
                    "label": label,
    ...
    def build_data_premise_and_one_hypo(
        ...
        with open(dump_path, "w", encoding="utf-8") as writer:
            for idx, sample in enumerate(data):
                ...
                label = sample["label"]
                input_ids, input_mask, type_ids = feature_extractor(
                    tokenizer, premise, text_b=hypothesis, max_length=max_seq_len
                )
                features = {
                    "uid": ids,
                    "label": label,
    ...
    def build_data_sequence(
        data, dump_path, max_seq_len=MAX_SEQ_LEN, tokenizer=None, label_mapper=None
    ):
        with open(dump_path, "w", encoding="utf-8") as writer:
            for idx, sample in enumerate(data):
                ids = sample["uid"]
