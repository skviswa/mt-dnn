...
def compare_output(output_dir, expected_dir):
    ...
    expected_train_loss = TRAIN_LOSS_RE.findall(
        open(os.path.join(expected_dir, "log.txt")).read()
    )
    assert (
    ...
    for file_name in (
        ...
        assert_file_equal(
            os.path.join(output_dir, file_name), os.path.join(expected_dir, file_name)
        )
