...
class PoolConfig(object):
    ...
    def from_dict(cls, json_object):
        ...
        config.shrink_size = (
            config.hidden_size
            if (config.shrink_size is None or config.shrink_size < 0)
            else config.shrink_size
        )
        return config
...
class ContextPooler(nn.Module):
    def __init__(self, config):
        ...
        self.with_ln = (
            config.context_layernorm if hasattr(config, "context_layernorm") else False
        )
        self.context_drop = (
            config.context_drop if hasattr(config, "context_drop") else 0
        )
        if self.with_ln:
...
class AttentionBlock(nn.Module):
    ...
    def forward(
        self, query_tensors, key_tensors, value_tensors, attention_mask, query_mask=None
    ):
        ...
        try:
            ...
            attention_probs = MaskedSoftmax(dim=-1)(
                attention_scores, attention_mask.to(torch.int32)
            )
...
class AttentivePooler(nn.Module):
    def __init__(self, config):
        ...
        if config.project_size > 0:
            ...
            self.project_dropout = StableDropout(
                config.hidden_dropout_prob
                if config.project_dropout_prob is None
                else config.project_dropout_prob
            )
    ...
    def forward(self, hidden_states, type_ids, input_mask):
        with torch.no_grad():
            ...
            attention_mask = (
                attention_mask.unsqueeze(1).unsqueeze(2).to(hidden_states.device)
            )
        ...
        if self.config.intermediate_size > 0:
            ...
            if pooled_output.size(-1) == output.size(-1):
                output = MaskedLayerNorm(
                    self.LayerNorm, self.dropout(output) + pooled_output
                )
        else:
...
class DualAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        mixed_key_layer = (
            self.key_a(key_tensors) * type_a_mask
            + self.key_b(key_tensors) * type_b_mask
        )
        mixed_value_layer = (
            self.value_a(value_tensors) * type_a_mask
            + self.value_b(value_tensors) * type_b_mask
        )
        # mixed_value_layer = value_tensors
