...
class BertSelfOutput(nn.Module):
    ...
    def forward(self, hidden_states, input_tensor, mask=None):
        ...
        hidden_states = ACT2FN[self.config.att_act](self.dropout(hidden_states))
        hidden_states = MaskedLayerNorm(self.LayerNorm, hidden_states + input_tensor)
...
class BertAttention(nn.Module):
    ...
    def forward(self, input_tensor, attention_mask, return_att=False):
        self_output = self.self(input_tensor, attention_mask, return_att)
        if return_att:
...
class BertLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, return_att=False):
        attention_output = self.attention(hidden_states, attention_mask, return_att)
        if return_att:
...
class BertEncoder(nn.Module):
    ...
    def forward(
        ...
        for layer_module in self.layer:
            hidden_states = layer_module(hidden_states, attention_mask, return_att)
            if return_att:
...
class BertModel(PreTrainedBertModel):
    ...
    def forward(
        ...
        if return_att:
            return encoded_layers, pooled_output, att_matrixs
