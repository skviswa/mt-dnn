...
class _ABCDataset(Dataset):
    ...
    def _make_inputs(self, src_left, src_right, pronoun_tokens, candidates, selected):
        lm_i, lm_s, lm_t = self._make_inputs_lm(
            src_left, src_right, pronoun_tokens, candidates, selected, 1
        )
        sm_i, sm_s, sm_t = self._make_inputs_sm(
            src_left, src_right, pronoun_tokens, candidates, selected, 0
        )
        input_ids = []
        task_ids = []
        if "lm" in self.tid:
    ...
    def _make_inputs_lm(
        self, src_left, src_right, pronoun_tokens, candidates, selected, tid=1
    ):
        ...
        for cand in candidates:
            if cand:
                ...
                cand_ids[
                    pronoun_idx : pronoun_idx + len(cand)
                ] = self.tokenizer.convert_tokens_to_ids(cand)
    ...
    def _make_inputs_sm(
        self, src_left, src_right, pronoun_tokens, candidates, selected, tid=0
    ):
        ...
        for cand in candidates:
            if cand:
                ...
                input_ids.append(
                    [token_ids, mask_ids, type_ids, cand_mask, pronoun_mask.copy()]
                )
            else:
                input_ids.append([[0], [0], [0], [0], [0]])
...
WSCRecord = namedtuple(
    "WSCRecord", ["sentence", "pron_idx", "pron", "selected", "candidates"]
)
...
class WSCDataset(_ABCDataset):
    ...
    def get_example(self, index):
        ...
        selected_idx = [
            i
            for i, c in enumerate(data.candidates.split(","))
            if c.lower().strip() == data.selected.lower().strip()
        ]
        ...
        return self._make_inputs(
            src_left, src_right, pronoun_tokens, candidates, selected
        )


WSC273Record = namedtuple(
    "WSC273Record", ["left", "pron", "right", "candidates", "selected"]
)
...
class WSC273Dataset(_ABCDataset):
    ...
    def __init__(
        self, tokenizer, datapaths, max_len=256, tid=None, topn=-1, max_candidates=2
    ):
...
WNLIRecord = namedtuple(
    "WNLIRecord",
    ["sentence", "hypersis", "pron_idx", "pron", "selected", "candidates", "label"],
)
...
class WNLIDataset(_ABCDataset):
    ...
    def __init__(
        self, tokenizer, datapaths, is_test=False, max_len=256, tid=None, topn=-1
    ):
    ...
    def get_example(self, index):
        ...
        selected = self.tokenizer.tokenize(
            convert_to_unicode(data.selected).lower().strip()
        )
        selected_idx = 0
...
GAPRecord = namedtuple(
    "GAPRecord", ["sentence", "pron_idx", "pron", "selected", "candidates"]
)
...
class GAPDataset(_ABCDataset):
    ...
    def get_example(self, index):
        ...
        src_right = self.tokenizer.tokenize(
            convert_to_unicode(data.sentence[pidx + len(data.pron) :])
        )
...
class DPRDTask(_ABCTask):
    ...
    def _mk_eval(self, name, data_dir, data, maxlen, input_type=None):
        ...
        return EvalData(name=name, examples=dataset, metrics_fn=self.get_metric_fn())
...
class WSC273Task(_ABCTask):
    ...
    def get_dev_dataset(self, data_dir, maxlen, input_type=None):
        eval_set = [
            self._mk_eval(
                "wsc273-test", data_dir, "wsc273.tsv", maxlen, input_type=input_type
            ),
            self._mk_eval(
                "pdp60-test",
                data_dir,
...
class WNLITask(_ABCTask):
    ...
    def _mk_eval(
        ...
        paths = glob(os.path.join(data_dir, data))
        dataset = WNLIDataset(
            self.tokenizer, paths, max_len=maxlen, is_test=is_test, tid=input_type
        )
        if name != "wnli-rank":
        ...
        return EvalData(
            name=name,
            examples=dataset,
            metrics_fn=self.get_metric_fn(scan_thred, dataset, rank),
            predict_fn=pred_fn,
        )
    ...
    def _rank_fn(self, datas):
        def predict_fn(logits, output_dir, name, prefix, *argv, **kwargs):
            ...
            if "tag" in kwargs:
                tag = kwargs["tag"]
            output = os.path.join(
                output_dir, "submit-rank-{}-{}-{}.tsv".format(name, prefix, tag)
            )
    ...
    def _predict_fn(self, datas):
        def predict_fn(logits, output_dir, name, prefix, *argv, **kwargs):
            ...
            th = (
                self.threshold
                if not tag or tag not in self.thred_dict
                else self.thred_dict[tag]
            )

            logger.info(f"Predict with [{name}][prefix][{tag}] {th:0.02}")
            output = os.path.join(
                output_dir, "submit-{}-{}-{}{:0.02}.tsv".format(name, prefix, tag, th)
            )
            with open(output, "w") as fs:
                fs.write("index\tpred\n")
                for i, l in enumerate(logits):
            ...
            for i in range(len(datas)):
                ...
                if group is None:
                    ...
                    group[-1][0].sentence == d.sentence
                    and group[-1][0].pron_idx == d.pron_idx
                ):
                    group.append([d, logits[i], i])
                else:
    ...
    def get_metric_fn(self, threshold_scan=False, data=None, rank=False):
        def metric_fn(logits, labels, *argv, **kwargs):
            ...
            if rank:
                return OrderedDict(
                    accuracy=self.wnli_accuracy(
                        logits, labels, threshold_scan, quiet, tag
                    ),
    ...
    def wnli_accuracy(self, logits, labels, threshold_scan, quiet=False, tag=None):
        ...
        if not quiet:
            logger.info(
                f"[{tag}] Mean sim score={np.mean(logits):0.02}|[+]{mp:0.02}|[-]{mn:0.02}; th={th:0.03}; acc@0.5={acc(0.5):0.02}"
            )
        if threshold_scan:
            ...
            if not quiet:
                logger.info(
                    f"[{tag}] Best score: BWD={score:0.03}@{th:0.02}, FWD={score2:0.03}@{th2:0.02}, Avg={score:0.03}@{best_th:0.02}"
                )
...
class GAPTask(_ABCTask):
    ...
    def get_dev_dataset(self, data_dir, maxlen, input_type=None):
        eval_set = [
            ...
            self._mk_eval(
                "GAP-dev", data_dir, "dev_annotated.tsv", maxlen, input_type=input_type
            ),
        ]
        return eval_set
