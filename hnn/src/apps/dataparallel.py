...
class XParallelOptimizer:
    def __init__(self, model, optimizer_fn, grad_clip_norm=1.0):
        ...
        self.sub_optimizers = [
            DeviceOptimizer(self.replicas, p, i, optimizer_fn(p, max_grad_norm=0))
            for i, p in self.param_groups.items()
        ]
    ...
    def step(self, grad_scale=1):
        ...
        parallel_apply(
            [st for _ in self.sub_optimizers],
            self.sub_optimizers,
            devices=[g.device for g in self.sub_optimizers],
        )
...
class DeviceOptimizer:
    ...
    def backward(self):
        group_params = [
            [(n, m[n]) for n, p in self.param_group if m[n].grad is not None]
            for m in self.named_params
        ]
        grad_params = [g for g in group_params if len(g) > 0]
        assert all([len(g) == len(grad_params[0]) for g in grad_params]), [
            len(g) for g in grad_params
        ]
    ...
    def step(self, l2norm):
        self.optimizer.step()
        group_params = [
            (i, [(n, m[n]) for n, p in self.param_group])
            for i, m in enumerate(self.named_params)
        ]
        group_params = sorted(
            group_params, key=lambda x: x[0] if x[0] != self.device else -1
        )
...
def optimizer_factory(
    args, training_steps=None, init_spec=None, no_decay=["bias", "LayerNorm.weight"]
):
