...
class Cosine(torch.nn.Module):
    ...
    def forward(self, src, tgt):
        ...
        tgt = tgt.float()
        return (
            torch.matmul(src, tgt.transpose(2, 1))
            / (
                src.norm(p=2, dim=-1, keepdim=True)
                * tgt.norm(p=2, dim=-1, keepdim=True)
                + 1e-9
            )
        ).squeeze()
...
def binary_loss(x, labels, alpha=10, beta=0.5, gama=1):
    ...
    loss = (
        -torch.nn.functional.logsigmoid(gama * pos)
        - torch.nn.functional.logsigmoid(-neg * gama)
    ) / 2
    return loss
...
def binary_rank_loss(x, labels, alpha=10, beta=0.5, gama=1):
    ...
    loss = -(labels * logp).sum(-1) + alpha * torch.max(
        torch.zeros_like(pos), neg - pos + beta
    )
    return loss
...
class SSMatcher(torch.nn.Module):
    ...
    def __init__(
        ...
        assert pooling in [
            ...
            "cap",
            "ftp",
        ], "Only cap, mean, ftp are supported pooling methods."
    ...
    def forward(self, bert, input_ids, labels=None, return_att=False):
        ...
        token_ids, mask_ids, type_ids, candidate_masks, pronoun_mask = [
            x.squeeze(1).contiguous() for x in input_ids.split(1, dim=1)
        ]
        ...
        candidate_encoding, att_score = pooling_fn(
            ctx_layer, pronoun_encoding, candidate_masks
        )
        if return_att:
            att_matrixs.append(att_score)

        sim_score = self.similarity(candidate_encoding, pronoun_encoding).view(
            batch_size, -1
        )
        ...
        if self.training:
            ...
            # b x c x d
            cand_ebd = candidate_encoding.view(
                batch_size, -1, candidate_encoding.size(-1)
            )
            # bx1
    ...
    def calc_pronoun_encoding(self, context_layer, pronoun_mask):
        ...
        query = ctx
        att = torch.matmul(query, context_layer.transpose(2, 1)) / math.sqrt(
            query.size(-1)
        )
    ...
    # wwm 75.4
    # CAP
    def calc_candidate_encoding_cap(
        self, context_layer, pronoun_encoding, candidate_masks
    ):
        # bx1xd
        ctx = context_layer[:, 0, :].unsqueeze(1)
    ...
    # Mean pooling
    def calc_candidate_encoding_mean(
        self, context_layer, pronoun_encoding, candidate_masks
    ):
        # bx1xd
        ctx = context_layer[:, 0, :].unsqueeze(1)
    ...
    def calc_candidate_encoding_ftp(
        self, context_layer, pronoun_encoding, candidate_masks
    ):
        # bx1xd
        ctx = context_layer[:, 0, :].unsqueeze(1)
        idx = (
            torch.arange(candidate_masks.size(-1), 0, -1)
            .unsqueeze(0)
            .expand(candidate_masks.size())
            .to(candidate_masks)
        )
        idx = idx * candidate_masks
        _, first_idx = torch.max(idx, dim=1, keepdim=True)
        first_idx = first_idx.unsqueeze(-1).expand(
            [context_layer.size(0), 1, context_layer.size(-1)]
        )
...
class LMMatcher(torch.nn.Module):
    ...
    def __init__(self, bert, config, alpha=5, beta=0.1, gama=1):
        ...
        self.beta = beta
        self.gama = gama
        self.lm_predictions = BertLMPredictionHead(
            config, bert.embeddings.word_embeddings.weight
        )
    ...
    def forward(self, bert, input_ids, labels=None):
        ...
        if input_ids.size(1) > 2:
            is_expanded = True
        input_ids = input_ids.view(-1, *input_ids.size()[2:])
        input_ids, attention_mask, token_type_ids, input_labels, _ = [
            x.squeeze(1).contiguous() for x in input_ids.split(1, dim=1)
        ]
        ...
        label_logits = torch.gather(
            log_logits, dim=-1, index=input_labels.unsqueeze(-1).long()
        ).squeeze(-1)
        label_mask = input_labels != 0
        # bc
        pred_logits = label_logits.masked_fill_(input_labels == 0, 0).sum(
            -1
        ) / label_mask.to(label_logits).sum(-1)
...
class HNNClassifer(NNModule):
    ...
    def __init__(
        ...
        self.sm_matcher = SSMatcher(
            config, alpha[0], beta[0], gama[0], similarity, loss_type, pooling
        )

        lm_idx = 1 if len(alpha) > 1 else 0
        self.lm_matcher = LMMatcher(
            self.bert, config, alpha[lm_idx], beta[lm_idx], gama[lm_idx]
        )
    ...
    def forward(self, input_ids, tids, labels=None, return_att=False, group_tasks=True):
        ...
        group_tasks = (
            bool(group_tasks[0])
            if isinstance(group_tasks, torch.Tensor)
            else group_tasks
        )
        ...
        def extract_tasks(task):
            if group_tasks:
                task = (
                    task[:, 1].view(input_ids.size(0), -1).unsqueeze(-1).unsqueeze(-1)
                )
                task = task.expand(
                    (task.size(0), task.size(1), input_ids.size(-2), input_ids.size(-1))
                ).long()
                ...
                task_inputs = torch.gather(
                    input_ids, dim=0, index=input_idx
                ).contiguous()
                # if str(input_ids.device)=='cuda:0':
                #  pdb.set_trace()
        ...
        if group_tasks or sm_logits is None or lm_logits is None:
            ...
            if self.training:
                en_loss = rank_loss(
                    en_logits, labels, self.alpha[-1], self.beta[-1], self.gama[-1]
                )
                loss += en_loss.mean()
        if self.training:
