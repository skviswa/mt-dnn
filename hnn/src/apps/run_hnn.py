...
def train_model(args, device, n_gpu, model, train_data, eval_data, training_steps):
    ...
    if args.init_spec and args.do_train:
        init_spec = InitSpec.load(args.init_spec)
    optimizer, param_optimizer, t_total = create_optimizer(
        model, args, init_spec=init_spec, num_train_steps=training_steps
    )  # , no_decay=['bias', 'LayerNorm.weight', 'theta'])
    tr_loss = 0
    nb_tr_steps = 0
    n_epoch = 0
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            while not go_next:
                _, _, _, loss = model(
                    ...
                    tids,
                    label_ids,
                    group_tasks=torch.tensor([args.group_tasks] * len(input_ids)),
                )
                if n_gpu > 1:
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    if args.fp16:
                        ...
                        if is_nan:
                            logger.warn(
                                "Hit nan gradient with loss scale: {}:{}".format(
                                    loss_scale, loss_upper
                                )
                            )
                            nan_cnt += 1
                            zero_grad(model, param_optimizer)
                    ...
                    if global_step % 100 == 0:
                        ...
                        logger.info(
                            "[{:0.1f}%][{:0.2f}h] Steps={}, loss={}, examples={}, {:0.1f}s".format(
                                100 * global_step / t_total,
                                (t_total - global_step)
                                * (start - end)
                                / ((global_step - last_step) * 3600),
                                global_step,
                                tr_loss / nb_tr_steps,
                                nb_tr_examples,
                                end - start,
                    ...
                    if global_step % 10000 == 0:
                        swa.update()
                        best_metric, best_epoch = make_check_point(
                            args,
                            device,
                            swa.shadow(),
                            param_optimizer,
...
def make_check_point(
    ...
    save_path = os.path.join(
        args.output_dir, f"pytorch.model-{n_epoch:02}-{steps:06}.bin"
    )
    state_dict = OrderedDict([(n, p) for n, p in model.module.state_dict().items()])
    if args.fp16:
        state_dict.update(
            [
                (n[len("module.") :] if n.startswith("module.") else n, p)
...
def run_eval(args, model, device, eval_data, prefix=None):
    ...
    for eval_item in eval_data:
        ...
        eval_dataloader = SequentialDataLoader(
            eval_item.data,
        ...
        en_predicts = []
        labels = []
        for input_ids, tids, label_ids in tqdm(
            eval_dataloader, ncols=80, desc="Evaluating: {}".format(prefix)
        ):
            with torch.no_grad():
                sm_logits, lm_logits, en_logits, tmp_eval_loss = model(
                    input_ids, tids, label_ids
                )
        ...
        def calc_metric(predicts, mn="", quiet=False):
            ...
            output_predict_file = os.path.join(
                args.output_dir, "{}predict_results_{}_{}.txt".format(mn, name, prefix)
            )
...
def run_predict(args, model, device, test_data, prefix=None):
    ...
    for test_item in test_data:
        ...
        test_dataloader = SequentialDataLoader(
            test_item.data,
        ...
        lm_predicts = []
        en_predicts = []
        for input_ids, tids, _ in tqdm(
            test_dataloader, ncols=80, desc="Predicting: {}".format(prefix)
        ):
            with torch.no_grad():
        ...
        def pred(predicts, tag):
            output_test_file = os.path.join(
                args.output_dir, "test_results_{}_{}_{}.txt".format(name, prefix, tag)
            )
            logger.info(
                "***** Dump prediction results-{}-{}-{} *****".format(name, prefix, tag)
            )
...
def build_training_data(args, tokenizer, tasks):
    ...
    if args.wiki_data:
        ...
        train_data = wiki_task.get_train_dataset(
            args.wiki_data, args.max_seq_length, input_type=tasks
        )
    else:
        train_data = dprd_task.get_train_dataset(
            args.data_dir, args.max_seq_length, input_type=tasks
        )
        if args.dev_train:
            _data = dprd_task.get_dev_dataset(
                args.data_dir, args.max_seq_length, input_type=tasks
            )
            _data = [e.data for e in _data if e.name == "DPRD-test"][0]
            train_data = ConcatDataset([train_data, _data])
        if args.gap_data:
            gap_data = gap_task.get_train_dataset(
                args.gap_data, args.max_seq_length, input_type=tasks
            )
            train_data = ConcatDataset([train_data, gap_data])
            if args.dev_train:
                gap_data = [
                    e.data
                    for e in gap_task.get_dev_dataset(
                        args.gap_data, args.max_seq_length, input_type=tasks
                    )
                ]
...
def main(args):
    ...
    if args.wnli_data:
        ...
        wnli_data = wnli_task.get_dev_dataset(
            args.wnli_data, 128, input_type=args.tasks
        )
        eval_data += wnli_data
    if args.wsc273_data:
        wsc273_task = WSC273Task(tokenizer)
        wsc273_data = wsc273_task.get_dev_dataset(
            args.wsc273_data, 128, input_type=args.tasks
        )
    ...
    if args.do_predict and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        test_data = dprd_task.get_test_dataset(
            args.data_dir, 128, input_type=args.tasks
        )
        if args.wnli_data:
            wnli_data = wnli_task.get_test_dataset(
                args.wnli_data, 128, input_type=args.tasks
            )
            test_data += wnli_data
        if args.wsc273_data:
            wsc273_data = wsc273_task.get_test_dataset(
                args.wsc273_data, 128, input_type=args.tasks
            )
...
def build_argument_parser():
    ...
    parser.add_argument(
        "--gama", default="1,1", type=str, help="Hyperparamter of ranking hinge loss"
    )
    parser.add_argument(
        "--alpha", default="10,10", type=str, help="Hyperparamter of ranking hinge loss"
    )
    ...
    parser.add_argument(
        "--wnli_data", default=None, type=str, help="data dir for wnli data"
    )

    parser.add_argument(
        "--wiki_data", default=None, type=str, help="data dir for wiki data"
    )

    parser.add_argument(
        "--wsrc_data", default=None, type=str, help="data dir for wsrc data"
    )

    parser.add_argument(
        "--gap_data", default=None, type=str, help="data dir for gap data"
    )
    ...
    parser.add_argument(
        "--wsc273_data", default=None, type=str, help="data dir for wsc273 data"
    )

    parser.add_argument("--tasks", default=None, type=str, help="tasks to run, SM,LM")
    ...
    parser.add_argument(
        "--swa", default=0, type=float, help="The parameter mu of model swa"
    )

    parser.add_argument(
        "--swa_start", default=0, type=float, help="The start step of model swa"
    )
    return parser
